...
initial value.
    ...
    let rec nats_from (start : int) : int stream =
      fun () -> Cons (start, nats_from (start + 1)) ;;
...
or, hiding the `nats_from` inside the definition of `nats`,
    ...
    let nats : int stream =
      let rec nats_from (start : int) : int stream =
        fun () -> Cons (start, nats_from (start + 1)) in
      nats_from 0 ;;
...
let rec alternate (s1 : 'a stream) (s2 : 'a stream) : 'a stream =
  fun () -> Cons (head s1, alternate s2 (tail s1)) ;;
